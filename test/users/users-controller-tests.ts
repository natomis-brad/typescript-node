...
describe("User Controller Tests", () => {
  ...
  it("Get User Info Unauthorized", (done) => {
    server.inject({
      method: 'GET',
      url: '/users/info',
      headers: {"authorization": "dummy token", "zincode-api-key": "abc"}
    }, (res) => {
  ...
  it("Get User Info Unauthorized wrong api key", (done) => {
    server.inject({
      method: 'GET',
      url: '/users/info',
      headers: {"authorization": "dummy token", "zincode-api-key": "cba"}
    }, (res) => {
      assert.equal(401, res.statusCode);
      done();
    });
